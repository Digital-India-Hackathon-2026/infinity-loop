...
def analyze_crop_image(image_bytes: bytes, crop_name: str) -> dict:
    ...
    if not (is_jpeg or is_png):
        raise ValueError("Unsupported file format. Only JPEG and PNG image formats are supported.")

    if not OPENCV_AVAILABLE:
        raise ValueError("OpenCV image processing engine is currently unavailable on the server.")
    ...
    try:
        ...
        if img is None:
            raise ValueError("Corrupted image file. The image could not be decoded.")

        # Resize large images to maximum dimension of 1024px before running analysis
        max_dim = 1024
        h, w = img.shape[:2]
        if max(h, w) > max_dim:
        ...
        raise ValueError(f"Visual quality analysis failed: {str(e)}")
